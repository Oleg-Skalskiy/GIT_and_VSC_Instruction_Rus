...
* Рекомендуем ознакомиться с инструкцией по установке и настройки GIT по ссылке ниже.
    ...
    * [FAQ](https://git-scm.com/book/ru/v2/)
...
* После установки выше перечисленного запускаем Visual Studio Code и немного настроим его.

    * Нажимаем последовательно кнопки на клавиатуре CTRL+P и вызовим командную строку для поска и ввода параметров. Ниже скрин как выглядит командная строка параметров.

    <img src="ctrl+p.png" width="1000" height="70" /> 

    * Вводим `>` для поиска параматеров и через пробел дописываем `Auto Save` и кликаем по надписи. Этим самым мы включим автосохранение.
    * Если Вы хотите изменить язык интерфейса то вводим `>` для поиска параматеров и через пробел дописываем `Configure Display Language` и кликаем по нему. После выбираем нужный язык. (Языковые файлы не предустановлены, поэтому придётся подождать, пока он их скачает)

    * Со всеми остальными параметрами можно ознакомится тут [FAQ](https://code.visualstudio.com/docs/supporting/faq)

## Познакомимся с основными (базовыми) командами GIT и настроим их

<u>**0 . Для начала запустим терминал (CTRL+`)**</u>
...
Можно также использовать маски файлов (например *.html) для удаления всех файлов, соответствующих критерию. Критерии можно создовать разные используя `*` как неизвестное
